$(document).ready(function(){
	...
	*/
	...
	$('.make_appt_btn').click(function(event){
		...
		var startTime = formatTime(apptTime);
		var endTime = getEndTime(apptTitle, startTime);
...
function formatTime(timeIn) {
	...
	if(AMorPM === 'PM') {
		...
		//alert("Returned military time: " + ret);
	}
...
function convertToMilitaryTime(timeIn) {
	...
	//specify base 10 since older browsers may convert a number that starts with 0 to octal base
...
function addMilitaryTime(length, startTime) {
	...
	if(newMinutes >= 60) {
		...
		hourNum += 1;
		ret = hourNum.toString() + ":" + dif.toString();
	}
	else {
		ret = hourNum.toString() + ":" + newMinutes.toString();
	}

	//TO-DO: fix if hours and/or minutes is 1 digit -> 9:0
	return ret;
}