...
/*
Returns the ID of a given customer. The ID is the primary key in the Customer
table, uniquely identifying each customer. This method is used when making an 
appointment, since the customer ID is a column in the Appointment table,
and also to verify the customer name entered exists in the database.
*/
function getCustomerID(nameIn, callback){
	var xmlhttp = createXmlHttpRequestObject();
	...
	xmlhttp.onreadystatechange = function(){
		if((xmlhttp.readyState === 4) && (xmlhttp.status === 200)){
			console.log("CustomerID returned: " + xmlhttp.responseText);
			//Return either the customerID, or a <p> element displaying an error message
			var response = xmlhttp.responseText;

			//if the response is not a number, then no ID was returned
			//therefore, display HTML error
			if(isNaN(response)){
			...
			//callback is called regardless of response
			callback(response);
		}
	}

	xmlhttp.open("POST", "get_customer.php", true);
	//xmlhttp.open("POST", "get_customer.php", false);

	//indicates the type of data you are sending
	//most servers will determine the data type, but good practice to set this
	xmlhttp.setRequestHeader("Content-type", "application/x-www-form-urlencoded");
	var params = "name="+nameIn;
	xmlhttp.send(params);
}
