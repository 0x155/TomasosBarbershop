	...
	if(!collision){
		...
		if(numSelectedSrvcs > 0){
			...
			if(oldSrvcsArr.join() !== newSrvcsArr.join()){
				...
				console.log("Different services!!");
...
function submitNewEmployees(e) {
	...
	console.log("Name: " + employeeName.value + ", Cell: " + employeeCellNumber.value
	...
	if (employeeCellNumber.value.length > 0) {
		newEmployee.cellNumber = employeeCellNumber.value;
	}

	// Call AJAX function to add employee to DB
	addNewEmployee(newEmployee, function() {
		alert("DONE!");
	});

	// Callback function appends the new empoyee to nullEmployeeList
}
