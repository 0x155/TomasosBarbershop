...
$(document).ready(function() {
	...
	// The close button is included in the fullCustomerHistory HB template
	// Need to bind the event to #customer_history since that is a DOM element
	// which is included in the DOM when the page loads.
	// Could have bound the event to document, but then the click event would
	// bubble up all the way to the document before realizing the function
	// should execute
	$("#customer_history").on('click', '.close_button', function(){
		$("#customer_history").html("");
	});

	// Doing same for buttons on the Add New Customer form template
	// These functions used to be called through the onClick attribute,
	// but those functions wont be exposed once all JS is modularized
	$("#new-customer-modal-container").on('click', '.close-customer-window', function() {
		closeNewCustomerWindow();
	});

	$("#new-customer-modal-container").on('click', '#clearNewCustFieldsButton', function() {
		clearAddNewCustomerFields();
	});

	$("#new-customer-modal-container").on('click', '#addNewCustomerButton', function() {
		addNewCustomer();
	});
	...
	*/
	...
	scheduler.showLightbox = function(id){
		...
		TODO - running this each time the lightbox is opened is inefficient
		*/
		...
		getListOfServices(id, function(servList){
			...
			if(serviceOptions.length > 0){
				...
				document.getElementById("lightbox-services-unavailable").style.display = "block";
			}
		});
	...
	*/
	...
	// Contains logic for double click on customer search results row
	// var lastTap;
...
function showSettings() {
	var doc = document,
			...
			unitEmployeeList = [],
			nullUnitEmployeeList = [],
			content,
			employees;

	// Seperates employees with a unit_id and those without
	getAllEmployeeNames(function(returnedEmployees) {
		returnedEmployees.forEach(function(employee){
			if (employee.Unit_ID) {
				unitEmployeeList.push(employee);
			}
			else {
				nullUnitEmployeeList.push(employee);
			}
		});

		employees = { unitEmployees: unitEmployeeList, nullUnitEmployees: nullUnitEmployeeList };
		content = Handlebars.templates.settingsModal(employees);
		$("#settings-modal-container").html(content);
	});
}
...
function closeSettings() {
	...
	$("#settings-modal-container").empty();
}
...
function submitNewEmployee(e) {
	...
	// Cell number is validated server-side
	...
	if (valid) {
		...
		addNewEmployee(newEmployee,
			function(response) {
				...
				$("input[type='text']", "#new-employee-form").val("");
			},
			function(error) {
...
function makeAppointment(){
	...
	if(fieldsLength){
		...
		TODO: Get/set start-time from either database, file, etc. not hardcoded.
...
function closeNewCustomerWindow(){
	$("#new-customer-modal-container").html("");
}
