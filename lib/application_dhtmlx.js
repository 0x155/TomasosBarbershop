	...
	*/
	scheduler.attachEvent("onEventCollision",  function (ev, evs){
		...
		if(scheduler.getState().lightbox_id){
			console.log("---Lightbox , Events in the time space:---");
			/*
			for (var i = evs.length - 1; i >= 0; i--) {
				console.log(typeof(evs[i].text) + "-> " + evs[i].start_date);
			}
			console.log("--------");
			*/
		}
	...
	//TO-DO: do nothing when user double clicks an empty cell
	//may need just JQuery, maybe no event through scheduler API
...
*/
	...
	*/
	...
	//If a user double clicks on a row in the customer search results modal, 
...
function toggleEditCustInfoFields(){
	...
	*/
	...
	//TO-DO: Toggle focus on the phone number field
