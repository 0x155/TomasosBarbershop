...
function openNewCustomerWindow(){
	var doc = document,
						enteredName = doc.getElementById("customer_name").value;
	//populate the name field in the Add Customer window with the name the user entered
	//so far in the Customer Name field

	//TODO - if the user is searching by phone number, pre-fill the phone number field
	// If the user is searching by name, then pre-fill the customer name field

	var content = Handlebars.templates.addNewCustomerForm({name: enteredName});
	$("#new-customer-modal-container").html(content);
	$("#new_cust_name").focus();
}

function closeNewCustomerWindow(){
	var noCustomerResuts = document.getElementById("no-customer-returned");
	if (noCustomerResuts) {
		noCustomerResuts.style.display = "none";
	}

	$("#new-customer-modal-container").empty();
}
