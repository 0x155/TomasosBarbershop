# Application Screenshots

### Main View of Application
---
![alt text](./../screenshots/main_page.png)

### Customer Dashboard View
---
![alt text](./../screenshots/customer_dashboard.JPG)

### Customer Full History View
---
![alt text](./../screenshots/customer_full_visit_history.png)

### New Customer Form
---
![alt text](./../screenshots/new_customer_form.JPG)

### Adding Services to an Appointment
##### Notice multiple services can be added for each appointment
---
![alt text](./../screenshots/services.JPG)

### Edit Appointment Lightbox
---
![alt text](./../screenshots/lightbox.png)

### Customer Search Results Window
---
![alt text](./../screenshots/search_results.png)

### Appointment Quick Info Window
---
![alt text](./../screenshots/quick_info.png)
